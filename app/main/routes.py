...
#TK: add context variables (session, g) and user login registration, login callback, load user data upon login, etc
...
#logout route
#instead of sayin app.route() we are saying main.route() because all the routes are contained in our main blueprint object (the blueprint is a global variable, like app used to be before we instantiated app inside the create_app() factory function in app/__init__.py file.
@main.route('/logout')
@login_required
def logout():
    logout_user()
    ...
    # what if two or more blueprints have an index route? Flask handles that by namespacing the endpoint names, which is why we need to say main.index to get to the specific views in a particular blueprint (which is main, as that is the only blueprint right now).
...
def protected():
    ...
    if tagform.validate_on_submit():
        ...
        #POST request sends this via tagform.tag object: <input id="tag" name="tag" type="text" value="THIS IS A TAG">
